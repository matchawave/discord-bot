...
pub use functions::*;
use permission::PermissionCallback;
pub use permission::{PermissionCheck, PermissionRequest};
...
use serenity::{
    all::{
        ...
        CreateMessage, GuildId, Message, PartialGuild,
    },
    async_trait,
    model::channel,
};
...
use crate::{
    ...
    event,
    extractors::{ContextEventExtractor, ContextExtractor, ExtractorTuple},
    global::Commands,
    guilds::{Channels, FakePerms, GuildMap, HTTPGetter, Members},
    handler::{CallbackReturn, HandlerBuilder, HandlerFn},
...
impl CommandManagerBuilder {
    ...
    pub fn set_permission_callback<F, U, Args>(mut self, callback: F) -> Self
    where
        F: HandlerFn<Args, U> + Send + Sync + Copy + 'static,
        Args: ExtractorTuple<PermissionRequest> + Send + Sync + 'static,
        U: CallbackReturn<Result<bool, ResponseError>> + 'static,
    {
        let handler = HandlerBuilder::<PermissionRequest, U>::build(callback);
        self.permission_callback = Some(Box::new(handler));
        self
    }
...
pub struct CommandManager {
    commands: Arc<HashMap<String, ICommand>>,
    permission_callback: Option<PermissionCallback>,
}
...
impl CommandExecution<Message> for CommandManager {
    async fn execute(
        ...
        let pre_event: CommandEvent = (&pre_action).into();

        let prefix_option = match Prefix::extract_context_event(ctx, (&pre_event).into()).await {
        ...
        if let Some(perms_callback) = &(self.permission_callback)
            && let event =
                PermissionRequest::new_user(guild_id, channel_id, user_id, command.permissions())
            && let Some(permission_result) = perms_callback.call(ctx, &event, &parser).await
            && let Err(permission_error) = permission_result
        {
        ...
        if let Some(perms_callback) = &(self.permission_callback)
            && let event = PermissionRequest::new_roles(
                guild_id,
                channel_id,
                &member.roles,
                command.permissions(),
        ...
        // Get the guild
        let Some(guild_ptr) = Pointer::<PartialGuild>::extract_context_event(ctx, &event).await
        else {
            return Err(ResponseError::new(format!(
        ...
        //     // Check if the user itself has the permission
        //     // Check if one of the roles has the permission
...
impl CommandExecution<CommandInteraction> for CommandManager {
    async fn execute(
        ...
        let parser_ptr = Pointer::new(Parser::new(ctx.shard_id));
        // ProcessManager for handling cooldowns and ephemerals
        let Some(p_manager) = Arc::<ProcessManager>::extract_context(ctx).await else {
            return Err(ResponseError::new(format!(
                "{command_prefix_print} Failed to extract ProcessManager from context"
        ...
        if let Some(perms_callback) = &(self.permission_callback)
            && let event =
                PermissionRequest::new_user(guild_id, channel_id, user_id, command.permissions())
            && let Some(permission_result) = perms_callback.call(ctx, &event, &parser_ptr).await
            && let Err(permission_error) = permission_result
        {
            return Err(permission_error);
        }
        ...
        if let Some(func) = command.slash()
            && let Some(response) = func.call(ctx, &event, &parser_ptr).await
        {
        ...
        Ok(Some((event, parser_ptr.clone())))
    }
}
